...
class bundo extends Controller
{
    ...
    public function add()
    {
        ...
        if ($request->getPost('nik') != NULL) {
            ...
            if (!$validation->withRequest($request)->run()) {
                ...
                return redirect()->to(base_url() . '/bundo/input');
            }
        }
        ...
        $cek = $this->model->caridata('Bundo Kanduang', $jbtn, $tgl_lantik, NULL, $nik);
        // dd($cek);
        if ($cek != NULL) {
        ...
        $data = array(
            'no' => date('y', strtotime($request->getPost('tgl_lantik'))) . '-04-' . $jbtn,
            'nik' => $nik,
    ...
    public function update()
    {
        ...
        if ($request->getPost('nik') != NULL) {
            ...
            if (!$validation->withRequest($request)->run()) {
                session()->setFlashdata('error', $validation->listErrors());
                return redirect()->to(base_url() . '/bundo/edit/' . $id_pemerintahan);
            }
        }
        ...
        $cek = $this->model->caridata('Bundo Kanduang', $jbtn, $tgl_lantik, $berhenti, $nik);
        // dd($cek);
        if ($cek != NULL) {
            session()->setFlashdata('error', 'Data sudah ada');
            return redirect()->to(base_url() . '/bundo/edit/' . $id_pemerintahan);
        ...
        $data = array(
            'no' => date('y', strtotime($tgl_lantik)) . '-04-' . $jbtn,
            'nik' => $nik,
            'nama' => $request->getPost('nama'),
            'jabatan' => $jbtn,
            'status' => 'Bundo Kanduang',
    ...
    public function proses()
    {
        ...
        foreach ($sheet as $x => $excel) {
            ...
            if ($excel[5] == '' or $excel[5] == '0000-00-00' or $excel[5] == '-') {
                $berhenti = NULL;
                $ket = 'Menjabat';
                $jabatan = $this->model->cek('Bundo Kanduang', $excel[3], $excel[4], '0000-00-00');
                if ($jabatan != NULL) {
                    if ($excel[3] != 'Anggota') {
                        ...
                        $dup = $this->model->duplikat('Bundo Kanduang', 'Anggota', $excel[4], '0000-00-00');
                        $jml = 0;
            ...
            if ($jabatan != NULL) {
                if ($excel[3] != 'Anggota') {
                    ...
                    $dup = $this->model->duplikat('Bundo Kanduang', 'Anggota', $excel[4], $berhenti);
                    $jml = 0;
                    for ($i = 0; $i < count($dup); $i++) {
                        if ($excel[3] == $dup[$i]['jabatan'] and $excel[2] == $dup[$i]['nama']) {
                            $jml++;
